...
use crate::util::test_data::{external_identity, external_identity_provider, external_identity_raw, external_token, principal_type, user_name};
...
async fn it_can_issue_token() {
    ...
    let validator= Arc::new(AlwaysValid { identity: external_identity() });
    ...
    let token = token_service.issue_token(external_identity_provider(), external_token()).await.unwrap();
    assert_ne!(token, "");
}
